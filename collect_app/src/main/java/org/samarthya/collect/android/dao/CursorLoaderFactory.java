package org.samarthya.collect.android.dao;
...
public class CursorLoaderFactory {
    ...
    public CursorLoader createEditableInstancesCursorLoader1(String sortOrder, String formFormat) {
        String selection = DatabaseInstanceColumns.STATUS + " !=? " +
                "and " + DatabaseInstanceColumns.STATUS + " !=? and " + DatabaseInstanceColumns.DISPLAY_NAME + " LIKE ?";
        String[] selectionArgs = {
                Instance.STATUS_SUBMITTED,
                Instance.STATUS_SUBMISSION_FAILED,
                "%" + formFormat + "%"
    ...
    public CursorLoader createEditableInstancesCursorLoader(CharSequence charSequence, String sortOrder, String formFormat) {
        CursorLoader cursorLoader;
        if (charSequence.length() == 0) {
            cursorLoader = createEditableInstancesCursorLoader1(sortOrder, formFormat);
        } else {
            String selection = DatabaseInstanceColumns.STATUS + " !=? " +
                    "and " + DatabaseInstanceColumns.STATUS + " !=? " +
                    "and " + DatabaseInstanceColumns.DISPLAY_NAME + " LIKE ?";
    ...
    public CursorLoader createSavedInstancesCursorLoader(String sortOrder, String formFormat) {
        String selection = DatabaseInstanceColumns.DELETED_DATE + " IS NULL and "
                + DatabaseInstanceColumns.DISPLAY_NAME + " LIKE ?";
        String[] selectionArgs = {"%" + formFormat + "%"};
        return getInstancesCursorLoader(selection, selectionArgs, sortOrder);
    }

    public CursorLoader createSavedInstancesCursorLoader(CharSequence charSequence, String sortOrder, String formFormat) {
        CursorLoader cursorLoader;
        if (charSequence.length() == 0) {
            cursorLoader = createSavedInstancesCursorLoader(sortOrder, formFormat);
        } else {
            String selection =
    ...
    public CursorLoader createFinalizedInstancesCursorLoader(String sortOrder, String formFormat) {
        ...
        String[] selectionArgs = {Instance.STATUS_COMPLETE, Instance.STATUS_SUBMISSION_FAILED, "%"+formFormat+"%"};

        return getInstancesCursorLoader(selection, selectionArgs, sortOrder);
    }

    public CursorLoader createFinalizedInstancesCursorLoader(CharSequence charSequence, String sortOrder, String formFormat) {
        CursorLoader cursorLoader;
        if (charSequence.length() == 0) {
            cursorLoader = createFinalizedInstancesCursorLoader(sortOrder, formFormat);
        } else {
            String selection =
    ...
    public CursorLoader createCompletedUndeletedInstancesCursorLoader(String sortOrder, String formFormat) {
        String selection = DatabaseInstanceColumns.DELETED_DATE + " IS NULL and ("
                + DatabaseInstanceColumns.STATUS + "=? or "
                + DatabaseInstanceColumns.STATUS + "=? or "
                + DatabaseInstanceColumns.STATUS + "=?) and " + DatabaseInstanceColumns.DISPLAY_NAME +" LIKE ?";

        String[] selectionArgs = {Instance.STATUS_COMPLETE,
                Instance.STATUS_SUBMISSION_FAILED,
                Instance.STATUS_SUBMITTED, "%"+formFormat+"%"};

        return getInstancesCursorLoader(selection, selectionArgs, sortOrder);
    }

    public CursorLoader createCompletedUndeletedInstancesCursorLoader(CharSequence charSequence, String sortOrder, String formFormat) {
        CursorLoader cursorLoader;
        if (charSequence.length() == 0) {
            cursorLoader = createCompletedUndeletedInstancesCursorLoader(sortOrder, formFormat);
        } else {
            String selection = DatabaseInstanceColumns.DELETED_DATE + " IS NULL and ("
                    + DatabaseInstanceColumns.STATUS + "=? or "
                    + DatabaseInstanceColumns.STATUS + "=? or "
    ...
    /* Overloaded function */
    public CursorLoader getFormsCursorLoader(CharSequence charSequence, String sortOrder, boolean newestByFormId, String formFormat) {
        CursorLoader cursorLoader;
        ...
        if (charSequence.length() == 0) {
            Uri formUri = newestByFormId ?
                    FormsContract.getContentNewestFormsByFormIdUri(currentProjectProvider.getCurrentProject().getUuid()) :
                    FormsContract.getUri(currentProjectProvider.getCurrentProject().getUuid());
            cursorLoader = new CursorLoader(Collect.getInstance(), formUri, null, DatabaseFormColumns.DELETED_DATE + " IS NULL AND "+DatabaseFormColumns.DISPLAY_NAME + " LIKE ?", new String[]{"%"+formFormat+"%"}, sortOrder);
        } else {
            String selection = DatabaseFormColumns.DISPLAY_NAME + " LIKE ? AND " + DatabaseFormColumns.DELETED_DATE + " IS NULL";
            String[] selectionArgs = {"%" + charSequence + "%"};
