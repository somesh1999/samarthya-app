package org.samarthya.collect.android.database.instances;
...
import org.samarthya.collect.android.database.instances.InstanceDatabaseMigrator;
...
import static org.samarthya.collect.android.database.DatabaseConstants.INSTANCES_TABLE_NAME;
import static org.samarthya.collect.android.database.DatabaseObjectMapper.getInstanceFromCurrentCursorPosition;
import static org.samarthya.collect.android.database.DatabaseObjectMapper.getValuesFromInstance;
import static org.samarthya.collect.android.database.instances.DatabaseInstanceColumns.CAN_EDIT_WHEN_COMPLETE;
...
import static org.samarthya.collect.android.database.instances.DatabaseInstanceColumns.SUBMISSION_URI;
...
public final class DatabaseInstancesRepository implements InstancesRepository {
    ...
    public int countForms(String[] status){
        StringBuilder selection = new StringBuilder(DISPLAY_NAME + " LIKE ? and ("+STATUS + "=?");
        for (int i = 1; i < status.length; i++) {
            selection.append(" or ").append(STATUS).append("=?");
        }
        selection.append(")");

        return query(null, selection.toString(), status, null).getCount();
    }
