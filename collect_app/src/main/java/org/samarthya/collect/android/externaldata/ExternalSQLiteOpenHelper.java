...
import org.samarthya.collect.android.database.AltDatabasePathContext;
import org.samarthya.collect.android.exception.ExternalDataException;
import org.samarthya.collect.android.externaldata.ExternalDataReader;
import org.samarthya.collect.android.externaldata.ExternalDataUtil;
...
public class ExternalSQLiteOpenHelper extends SQLiteOpenHelper {
    ...
    private org.samarthya.collect.android.externaldata.ExternalDataReader externalDataReader;
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        try {
            if (shouldUpdateDBforDataSet(db, org.samarthya.collect.android.externaldata.ExternalDataUtil.EXTERNAL_DATA_TABLE_NAME, org.samarthya.collect.android.externaldata.ExternalDataUtil.EXTERNAL_METADATA_TABLE_NAME, dataSetFile)) {
                onCreateNamed(db, org.samarthya.collect.android.externaldata.ExternalDataUtil.EXTERNAL_DATA_TABLE_NAME);
                createAndPopulateMetadataTable(db, org.samarthya.collect.android.externaldata.ExternalDataUtil.EXTERNAL_METADATA_TABLE_NAME, dataSetFile);
            }
        } catch (Exception e) {
    ...
    private void onCreateNamed(SQLiteDatabase db, String tableName) throws Exception {
        ...
        try {
            ...
            if (!org.samarthya.collect.android.externaldata.ExternalDataUtil.containsAnyData(headerRow)) {
                throw new ExternalDataException(
                        TranslationHandler.getString(Collect.getInstance(), R.string.ext_file_no_data_error));
            }

            List<String> conflictingColumns =
                    org.samarthya.collect.android.externaldata.ExternalDataUtil.findMatchingColumnsAfterSafeningNames(headerRow);
            ...
            for (int i = 0; i < headerRow.length; i++) {
                ...
                String safeColumnName = org.samarthya.collect.android.externaldata.ExternalDataUtil.toSafeColumnName(columnName,
                        columnNamesCache);
                if (safeColumnName.equals(org.samarthya.collect.android.externaldata.ExternalDataUtil.SORT_COLUMN_NAME)) {
            ...
            if (!sortColumnAlreadyPresent) {
                sb.append(", ");
                sb.append(org.samarthya.collect.android.externaldata.ExternalDataUtil.SORT_COLUMN_NAME).append(" real ");
            }
            ...
            for (String header : headerRow) {
                if (header.endsWith("_key")) {
                    String indexSQL = "CREATE INDEX " + header + "_idx ON " + tableName + " ("
                            + org.samarthya.collect.android.externaldata.ExternalDataUtil.toSafeColumnName(header, columnNamesCache) + ");";
            ...
            while (row != null && !isCancelled()) {
                ...
                if (!org.samarthya.collect.android.externaldata.ExternalDataUtil.containsAnyData(row)) {
                ...
                if (row.length < headerRow.length) {
                    row = org.samarthya.collect.android.externaldata.ExternalDataUtil.fillUpNullValues(row, headerRow);
                }

                ContentValues values = new ContentValues();
                if (!sortColumnAlreadyPresent) {
                    values.put(org.samarthya.collect.android.externaldata.ExternalDataUtil.SORT_COLUMN_NAME, rowCount + 1);
                }
    ...
    static void createAndPopulateMetadataTable(SQLiteDatabase db, String metadataTableName, File dataSetFile) {
        final String dataSetFilenameColumn = CustomSQLiteQueryBuilder.quoteIdentifier(org.samarthya.collect.android.externaldata.ExternalDataUtil.COLUMN_DATASET_FILENAME);
        final String md5HashColumn = CustomSQLiteQueryBuilder.quoteIdentifier(org.samarthya.collect.android.externaldata.ExternalDataUtil.COLUMN_MD5_HASH);
        ...
        metadata.put(org.samarthya.collect.android.externaldata.ExternalDataUtil.COLUMN_DATASET_FILENAME, dataSetFile.getName());
        metadata.put(org.samarthya.collect.android.externaldata.ExternalDataUtil.COLUMN_MD5_HASH, Md5.getMd5Hash(dataSetFile));
    ...
    static boolean shouldUpdateDBforDataSet(File dbFile, File dataSetFile) {
        ...
        return shouldUpdateDBforDataSet(db, org.samarthya.collect.android.externaldata.ExternalDataUtil.EXTERNAL_DATA_TABLE_NAME, ExternalDataUtil.EXTERNAL_METADATA_TABLE_NAME, dataSetFile);
    }
