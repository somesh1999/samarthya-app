...
import org.samarthya.collect.android.formmanagement.DiskFormsSynchronizer;
...
public class FormsDirDiskFormsSynchronizer implements DiskFormsSynchronizer {
    ...
    public String synchronizeAndReturnError() {
        ...
        try {
            ...
            if (errors.length() != 0) {
                ...
                Timber.d(org.samarthya.collect.android.utilities.TranslationHandler.getString(Collect.getInstance(), R.string.finished_disk_scan));
            }
    ...
    private Form parseForm(File formDefFile) throws IllegalArgumentException {
        ...
        try {
            fields = org.samarthya.collect.android.utilities.FileUtils.getMetadataFromFormDefinition(formDefFile);
        ...
        String title = fields.get(org.samarthya.collect.android.utilities.FileUtils.TITLE);

        if (title != null) {
            builder.displayName(title);
        } else {
            throw new IllegalArgumentException(
                    org.samarthya.collect.android.utilities.TranslationHandler.getString(Collect.getInstance(), R.string.xform_parse_error,
                            formDefFile.getName(), "title"));
        }
        String formid = fields.get(org.samarthya.collect.android.utilities.FileUtils.FORMID);
        if (formid != null) {
        ...
        String version = fields.get(org.samarthya.collect.android.utilities.FileUtils.VERSION);
        if (version != null) {
            builder.version(version);
        }
        String submission = fields.get(org.samarthya.collect.android.utilities.FileUtils.SUBMISSIONURI);
        if (submission != null) {
        ...
        String base64RsaPublicKey = fields.get(org.samarthya.collect.android.utilities.FileUtils.BASE64_RSA_PUBLIC_KEY);
        if (base64RsaPublicKey != null) {
            builder.base64RSAPublicKey(base64RsaPublicKey);
        }
        builder.autoDelete(fields.get(org.samarthya.collect.android.utilities.FileUtils.AUTO_DELETE));
        builder.autoSend(fields.get(org.samarthya.collect.android.utilities.FileUtils.AUTO_SEND));
        builder.geometryXpath(fields.get(org.samarthya.collect.android.utilities.FileUtils.GEOMETRY_XPATH));
